...
class Hexa_Path:
    ...
    def Initial_Control(self):
        for i in range(4):
            if i < len(self.Predict_BlockValue):
        ...
        Top.config(bg="white")
        try:
            icon_img = Image.open("hexa_dice.png")
            icon_photo = ImageTk.PhotoImage(icon_img)
            Top.wm_iconphoto(False, icon_photo)
        except Exception as e:
            print("The icon hasn't been downloaded", e)
            
        Head = Label(Top, text="Total num of Players", font=("Times new roman", 30, "bold", "italic"), bg="white")
        Head.place(x=50, y=30)
            
            
        Entry_take = Entry(Top, font=("Times new roman", 18, "bold", "italic"), relief=SUNKEN, bd=5, width=12, state=NORMAL)
        ...
        btn_Submit = Button(Top, text="Submit", bg="#262626", fg ="white", font=("Times new roman", 13, "bold"), relief=RAISED, bd=3, command=Filter_value, state=NORMAL)
        ...
        def Operate_computer(ind):
            if ind:
                ...
                Time_is = 5
                Place_ins = Label(Top, text="", font=("Times new roman", 20, "bold"), fg="#FF0000", bg="white")  
                Play_Command = Label(Top, text="", font=("Helvetica", 12, "bold"), fg="blue", bg="white")
                ...
                if len(self.Predict_BlockValue) > 1:
...
if __name__ == '__main__':
    ...
    window.title("Hexa Path")
    
    
    try:
        icon_img = Image.open(resource_path("images/hexa_dice.png"))
        icon_photo = ImageTk.PhotoImage(icon_img)
        window.wm_iconphoto(False, icon_photo)
    except Exception as e:
        print("لم يتم تحميل الأيقونة، سيتم تشغيل اللعبة بدونها:", e)

   
    try:
        block_six_side = ImageTk.PhotoImage(Image.open(resource_path("images/dice1.png")).resize((33, 33), Image.Resampling.LANCZOS))
        block_five_side = ImageTk.PhotoImage(Image.open(resource_path("images/dice2.png")).resize((33, 33), Image.Resampling.LANCZOS))
        block_four_side = ImageTk.PhotoImage(Image.open(resource_path("images/dice3.png")).resize((33, 33), Image.Resampling.LANCZOS))
        block_three_side = ImageTk.PhotoImage(Image.open(resource_path("images/dice4.png")).resize((33, 33), Image.Resampling.LANCZOS))
        block_two_side = ImageTk.PhotoImage(Image.open(resource_path("images/dice5.png")).resize((33, 33), Image.Resampling.LANCZOS))
        block_one_side = ImageTk.PhotoImage(Image.open(resource_path("images/dice6.png")).resize((33, 33), Image.Resampling.LANCZOS))
        
        
        Hexa_Path(window, block_six_side, block_five_side, block_four_side, block_three_side, block_two_side, block_one_side)
    
    except FileNotFoundError as e:
        print(f"خطأ: تأكد من وجود ملفات الصور (dice1.png إلى dice6.png) في نفس المجلد! \nالتفاصيل: {e}")
